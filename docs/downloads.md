...
- **Perturb-CITE-seq (SCP1064)** (`configs/download/perturb_cite_seq_urls.tsv`)
  - Slurm job: `12833199` (array 0–14%10; partition `io`; time 48:00:00; `CURL_INSECURE=1`)

## Notes

- SCP portal downloads require SSL workaround on this cluster (`CURL_INSECURE=1`). The job `12833136` failed because the manifest included a portal `manifest` URL; resubmitted as `12833174` after removing that entry. New auth_code issued; resubmitted as `12833199`.
- Sci-Plex4 corruption fixed by deleting the bad file and resubmitting a single-file download (job `12833198`).
